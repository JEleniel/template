...
- **Comment Formatting:**
    ...
    * You MUST conform to the [The rustdoc book](https://doc.rust-lang.org/rustdoc/). If modifications are made to documentation comments, you MUST use the `cargo doc` to generate and update documentation in `target/doc/` and copy that folder to `docs/rustdoc/`. You MUST NOT put the copy in a `doc` subfolder (e.g., `docs/rustdoc/doc/`).
...
- Unimplemented code paths MUST fail fast and be explicit (prefer `todo!();`, `unimplemented!();`, or similar). Unused variables must be prefixed with a underscore (`_`). Both unimplemented code and unused varaibles MUST include a comment explaining the intended future use or implementation.
- You MUST NOT use `unwrap`, `expect`, `panic`, or similar unless explicitely instructed by the user. You MUST use `thiserror` and `anyhow` for error handling:
...
- Include a single reverse-DNS app ID in the `Config.toml` as `app_id = "{Application ID}"`.
...
- `chrono` for time and date handlind - use RFC 3339 with millisecodn resolution when converting to strings.
...
- `log`, and either `fern` (preferred) or `tracing` (and sublibraries) for logging
    * Configure logging to write `TRACE`, `DEBUG`, `INFO`, and `WARN` to `stdout` and `ERROR` to stderror
